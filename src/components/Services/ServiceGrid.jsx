...
const ServiceGrid = ({ services, serviceCategories, onServiceClick }) => {
  ...
  // State: expanded/collapsed for each category
  ...
  // We'll use a custom class for scrollbar visibility on hover
  ...
  return (
    <div className="space-y-12">
      {Object.entries(servicesByCategory).map(([categoryId, categoryServices]) => {
        ...
        // Height logic: Increase height so scroll-y is never needed (420px + padding)
        // The parent .activity-scroll-x will handle horizontal scroll on hover only
        ...
        return (
          <div key={categoryId} className="space-y-6">
            ...
            {/* Services Grid - Horizontal scroll only on hover */}
            <div
              className="activity-scroll-x flex gap-4 sm:gap-6 pb-2"
              style={{
                minHeight: '420px', // Card height + padding, prevents y-scroll
                overflowY: 'visible', // always visible, never scroll
                // overflowX handled by .activity-scroll-x class
              }}
            >
              {visibleServices.map((service) => (
                <div
                  key={service.id}
                  className="bg-black rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl group relative border border-gray-800 hover:border-[#00FFAB]/50 h-[400px] min-w-[280px] w-[280px] max-w-[280px] flex flex-col"
              ...
              {hasMoreThanFour && !isExpanded && (
                <div
                  key="see-more"
                  className="bg-gradient-to-br from-[#00FFAB] to-green-400 rounded-2xl flex items-center justify-center cursor-pointer h-[400px] min-w-[280px] w-[280px] max-w-[280px] border border-gray-800 hover:border-[#00FFAB]/50 transition-all duration-300 hover:scale-105"
